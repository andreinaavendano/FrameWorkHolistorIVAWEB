...
class HomePageHolistorSYJ(BasePage):
    SYJ_IMAGEN_BUTTON = (By.XPATH, "//img[@src='/assets/common/images/holistor/SYJ-logo.png']")
    ...
    SYJ_IMAGEN_LOGO_HP = (By.ID, "K2BAPPLICATIONICON_MPAGE")
    ...
    def is_SYJ_tenant(self):
        if self.wait_for_element(self.SYJ_IMAGEN_BUTTON).is_displayed():
            return True
        else:
            return False

    def is_SYJ_user_enabled(self):
        return self.is_disabled(self.SYJ_IMAGEN_BUTTON)

    def open_syj(self):
        ventana_plataforma = self.get_name_handle(self.driver)
        self.click(self.SYJ_IMAGEN_BUTTON)
        self.swtich_page(ventana_plataforma, self.driver)
        return self.wait_for_element(self.SYJ_IMAGEN_LOGO_HP).is_displayed()
